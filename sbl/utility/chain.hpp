...
/**
 * @file chain.cpp
 * @brief A vector like container which can be insert and delete element in log time. (Deprecated)
 * @author Tianjiao Yin
 * @version 1.1
 * @date 2008-06-28
 */
...
template<class value_type> class Chain {
    ...
    public:
        ...
        Chain() {
            if (null_node == 0)null_node = new node(0);
            root = new node(2);
            node *k = new node(2);
            root->right = k;
            k->father = root;
    ...
    private:
        ...
        void Chain<value_type>::segment(int l, int r);
        friend class ChainNode<value_type>;
};

template<class value_type> ChainNode<value_type> *Chain<value_type>::null_node;

template<class value_type> int Chain<value_type>::size(const iterator &x) {
    node *p = x.p;
    int k = p->left->size;
    while (p != root)
        if (p->father->right == p) {
...
template<class value_type> value_type &Chain<value_type>::operator[](int k) {
    node *p = root;
    k += 2;
    while (p != null_node) {
        p->expand();
...
template<class value_type> void Chain<value_type>::rotate(node *x) {
    node *y = x->father;
...
template<class value_type> void Chain<value_type>::splay(node *t) {
...
template<class value_type> void Chain<value_type>::segment(int l, int r) {
    (*this)[l - 1];
    root->expand();
    node *t = root;
    node *k = t->right;
...
template<class value_type> void Chain<value_type>::remove(node *&t) {
    t->father = null_node;
    node *p = t, *q;
    while (p != null_node)
...
template<class value_type> ChainNode<value_type>::ChainNode<value_type>(int mode, const value_type &x): element(x) {
    if (mode == 0) {
        father = left = right = this;
        make_same = reverse = size = 0;
...
template<class value_type> void ChainNode<value_type>::Make_same(value_type &l) {
    ...
    make_same = 1;
    sum = l * size;
    max_sum = max_right_sum = max_left_sum = max(sum, l);
}

template<class value_type> void ChainNode<value_type>::Reverse() {
    reverse ^= 1;
    swap(max_left_sum, max_right_sum);
}

template<class value_type> void ChainNode<value_type>::expand() {
    if (make_same) {
...
template<class value_type> void ChainNode<value_type>::update() {
